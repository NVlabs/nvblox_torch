...
def show_renders(mapper, pose, name, timer, mapper_id):
    ...
    depth, color = mapper.render_depth_and_color_image(
        ...
        intrinsics=torch.tensor(
            [[480, 0, 320], [0, 480, 320], [0, 0, 1]], dtype=torch.float32
        ),
        height=480,
        width=640,
        max_ray_length=10.0,
...
def show_voxels(visualizer, mapper, source="esdf"):
    ...
    if source == "occupancy":
        ...
        sdf_viz = (
            surface_tsdf.clamp(-8 * voxel_size, 8 * voxel_size) + 8 * voxel_size
        ) / (16 * voxel_size)
        colors = (
            (
                torch.stack(
        ...
        raise ValueError(
            f"Unknown voxel source: {source}. Supported: esdf, tsdf, occupancy"
        )
...
def query_sdf_on_a_plane_grid(name, mapper, timer, mapper_id):
    ...
    xy = (
        torch.stack(torch.meshgrid(x, y, indexing="ij"))
        .permute((1, 2, 0))
        .reshape(-1, 2)
    )
