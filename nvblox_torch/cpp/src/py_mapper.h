...
struct Mapper : torch::CustomClassHolder {
  ...
  Mapper(std::vector<double> voxel_size_m, std::vector<std::string> projective_layer_type,
  ...
  c10::intrusive_ptr<Mapper> clone() const {
    return c10::make_intrusive<Mapper>(voxel_size_m_, projective_layer_type_, free_on_destruct_);
  }
